...
class CactiSimulation:
    """Interface to CACTI simulator to extract memory access energy, area, and latency."""
    ...
    def run_cacti(self):
        ...
        os.chdir(
            self.CACTI_PROGRAM_PATH
        )  # Change the directory to the cacti program directory
        stream = Popen(
    ...
    def _check_valid_config(self, mem_config):
        """Check if the provided mem_config is valid"""
        required_keys = [
            "technology",
            "mem_type",
            "size",
        ...
        if len(missed_keys) != 0:
            raise ValueError(
                f"missed keys {missed_keys} from the provided mem_config input"
            )
    ...
    def _prepare_config_file(self, cacti_config_file):
        ...
        mem_config["cache_size"] = int(
            self.mem_config["size"] / 8 / self.mem_config["bank_count"]
        )
        mem_config["IO_bus_width"] = int(
            self.mem_config["rw_bw"] / self.mem_config["bank_count"]
        )
        ...
        if (
            mem_config["IO_bus_width"]
            < (cacti_config_option["line_size"]["default"] * 8) / 2
        ):
            self.cacti_config.change_default_value(
                {"line_size": math.ceil(mem_config["IO_bus_width"] / 64) * 8}
            )

        for itm in cacti_config_option.keys():
            if itm in mem_config.keys():
                user_config.append(
                    cacti_config_option[itm]["string"] + str(mem_config[itm]) + "\n"
                )
            else:
                user_config.append(
                    cacti_config_option[itm]["string"]
                    + str(cacti_config_option[itm]["default"])
                    + "\n"
                )
